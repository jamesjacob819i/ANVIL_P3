...
async def handle_diagnostics_done(event: SentinelEvent):
    ...
    try:
        result = await run_rca(event.incident_id, event.payload)
        ...
        print(f"[rca_worker] Published rca.done for {event.incident_id}: confidence={result.get('confidence')}")
    except Exception as e:
        print(f"[rca_worker] Error processing {event.incident_id}: {e}")
...
async def main():
    bus = EventBus()
    await bus.connect()
    print("[rca_worker] Subscribing to diagnostics.done...")
    await bus.subscribe("diagnostics.done", "rca_worker_1", handle_diagnostics_done)
